...
def popuprule(rules):
    displayRules = [
        'Players needs to choose a number between 0 and 100. The average of the numbers everyone selected is multiplied by 0.8. The person who chose the number closest to that result is the winner. One point will be deducted from the remaining players, and the round is concluded. When a player reaches negative ten points, they will receive a game over. The last player standing will clear the game. However, every time a player is eliminated, a new rule is added. [Note: If two or more players are closest to average, the king wins the round by default]',
...
def game():
    ...
    if not st.session_state['player_names_entered']:
        ...
        # Ensure player names are distinct
        if len(set(player_names)) != len(player_names):
            st.warning("Player names must be distinct. Please re-enter unique names.")
        else:
        ...
        if not st.session_state.reset:
            if len(st.session_state['player']) > 1:
                ...
                if st.button("Confirm!"):
                    ...
                    # Clear the previous input fields (reset numbers in session state)
                    for name in st.session_state['player'].keys():
                        st.session_state[f"{name}_number"] = 0  # Reset the number input field
